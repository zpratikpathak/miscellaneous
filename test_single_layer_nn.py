...
def test_set_and_get_weights():
    ...
    assert weights.ndim == 2 and \
        weights.shape[0] == number_of_nodes and \
        weights.shape[1] == (input_dimensions + 1)
    assert np.array_equal(model.get_weights(), np.ones(
        (number_of_nodes, input_dimensions + 1)))
...
def test_weight_initialization():
    ...
    np.testing.assert_allclose(
        model.get_weights(), weights, rtol=1e-3, atol=1e-3)
...
def test_train_and_error_calculation():
    ...
    X_train = np.array([[0.60302512, -0.94856749, -0.88904878, -0.02272178, 0.90226341],
                        [0.23682103, -0.29573089, -0.2328635,
                            0.85537468, 0.19151025],
                        [-0.35294026, -0.99263065,
                            0.41645806, -0.22561292, 2.72811021],
                        [0.06180665, 1.0643302, 0.49739215, -1.81960612, 0.50104263],
                        [-0.4875518, -0.98996947, 2.38729703,
                            0.95753127, -0.20929545],
